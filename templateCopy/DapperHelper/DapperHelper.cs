...
namespace Dapper
{
        ...
        where TConnection : IDbConnection, new()
    {
        ...
        public string GenerateInsertSql<T>(T requestData, string tableName, string parameterPrefix = "@")
        {
            ...
            {
                columnNames.Add(prop.Name); // 將屬性名稱作為列名稱
                parameterNames.Add($"{parameterPrefix}{prop.Name}"); // 使用 parameterPrefix 符號 生成 SQL 參數
            }
        ...
        public string GenerateUpdateSql<T>(T requestData, string tableName, string primaryKey, bool hasWhereSql = true, string parameterPrefix = "@")
        {
            var properties = typeof(T).GetProperties();
            ...
            foreach (var prop in properties)
            {
                if (prop.Name != primaryKey) // 跳過主鍵欄位
                {
                    setClauses.Add($"{prop.Name} = {parameterPrefix}{prop.Name}"); // 使用指定前綴生成 SET 子句
                }
            }
            ...
            string sqlWhere = $"WHERE {primaryKey} = {parameterPrefix}{primaryKey}";
            if (hasWhereSql) sql += sqlWhere;

            return sql;
        }

        public string GenerateUpdateSql(List<string> columnsToUpdate, string tableName, string primaryKey, bool hasWhereSql = true, string parameterPrefix = "@")
        {
            var setClauses = new List<string>();

            foreach (var column in columnsToUpdate)
            {
                if (column != primaryKey) // 跳過主鍵欄位
                {
                    setClauses.Add($"{column} = {parameterPrefix}{column}"); // 使用指定前綴生成 SET 子句
                }
            }
            ...
            if (hasWhereSql)
            {
                sql += $"WHERE {primaryKey} = {parameterPrefix}{primaryKey}";
            }
